"""Lab 8.2 STARTER -- LMS / HSS hash-based signatures (RFC 8554, SP 800-208) in plain Python,
...
Complete the functions marked TODO. `PQ_LAB_IMPL=solution pytest labs/ch08/tests` runs the reference.
"""
...
def checksum(q: bytes, w: int, ls: int) -> bytes:
    """TODO: RFC 8554 section 4.4: sum over all w-bit digits of (2^w - 1 - digit), then u16(sum << ls)"""
...
def ots_pubkey_from_sig(message: bytes, sig: bytes, I: bytes, q: int) -> bytes:
    """TODO: Algorithm 4b: recompute Q and its checksum, hash each y[i] forward from digit a to 2^w-2, and hash I||q||D_PBLC||z[0..p-1]"""
    raise NotImplementedError("ots_pubkey_from_sig")
...
def lms_verify(pubkey: bytes, message: bytes, sig: bytes) -> bool:
    """TODO: Algorithm 6: parse q, the OTS signature, the LMS type and the path; recompute the leaf from the candidate OTS key; walk the path (even node: H(I,node/2,D_INTR,tmp,sib); odd: sib first); compare with the root"""
    raise NotImplementedError("lms_verify")
...
class CommittedLmsPrivateKey(LmsPrivateKey):
    ...
    def _take_index(self) -> int:
        """TODO: if q has reached the reservation boundary, persist a new boundary (fsync) BEFORE handing out any index; then return q and advance it."""
        raise NotImplementedError("CommittedLmsPrivateKey._take_index")
...
def forge_after_reuse(pubkey: bytes, reused: list[tuple[bytes, bytes]], target: bytes,
    ...
    """TODO: for each chain take the lowest revealed digit position across the reused signatures; try random C until every digit of the target is >= that position; hash forward to build y; reuse the path"""
    raise NotImplementedError("forge_after_reuse")
