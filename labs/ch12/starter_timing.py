"""Lab 12.2 STARTER -- a dudect-style timing-leak detector, applied to a deliberately leaky toy
...
Complete the three functions marked TODO. `PQ_LAB_IMPL=solution pytest labs/ch12/tests` runs the reference.
"""
...
def welch_t(a: list[float], b: list[float]) -> float:
    """TODO: (mean_a - mean_b) / sqrt(var_a/n_a + var_b/n_b) with population variances; 0 if the denominator is 0"""
    raise NotImplementedError("welch_t")



def crop(xs: list[float], pct: float = 0.9) -> list[float]:
    """TODO: sort ascending and keep the lowest pct fraction (at least 2 samples)"""
    raise NotImplementedError("crop")



def measure(fn, inputs_a: list, inputs_b: list, n: int, warmup: int = 200) -> dict:
    """TODO: warm up; build a shuffled list of 2n class labels; time fn on the next input of each class with perf_counter_ns; crop both; return dict(n_per_class, median_a_ns, median_b_ns, t (rounded 2), leak = |t| > T_THRESHOLD)"""
