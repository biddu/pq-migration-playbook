"""Lab 11.1 solution (part 2) -- policy-as-code, negotiation, the kill switch, rollback, telemetry.
...
class Policy:
    ...
    def usable(self, kind: str, now: dt.date) -> list[str]:
        """Our algorithms, in preference order, after kill switch, deprecation dates and floor."""
        pref, floor = (self.kem_preferred, self.kem_floor) if kind == "kem" else (self.sig_preferred, self.sig_floor)
        return [a for a in pref if not self.is_killed(a) and not self.is_disallowed(a, now)
                and providers.get(a).meta.quantum_level >= floor]

    # ---- change control
    def with_kill(self, alg: str) -> "Policy":
        new = copy.deepcopy(self); new.previous = self; new.version += 1
        if alg not in new.kill_switch:
            new.kill_switch.append(alg)
        return new
...
def negotiate(policy: Policy, kind: str, peer_offer: list[str], now: dt.date | None = None) -> Event:
    ...
    ours = policy.usable(kind, now)
    common = [a for a in ours if a in peer_offer]
    best_common_level = max((providers.get(a).meta.quantum_level for a in common), default=None)
    chosen = common[0] if common else None
    level = providers.get(chosen).meta.quantum_level if chosen else None
    ev = Event(dt.datetime.now(dt.timezone.utc).isoformat(timespec="seconds"), kind, policy.version, policy.digest(),
               ours, list(peer_offer), chosen, level,
               downgrade=bool(chosen) and level == 0 and (best_common_level or 0) > 0,
               warned=bool(chosen) and policy.is_warned(chosen, now),
               reason="ok" if chosen else ("no common algorithm" if ours else "policy leaves nothing usable"))
    TELEMETRY.append(ev)
    return ev
