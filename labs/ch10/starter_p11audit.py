"""Lab 10.1 STARTER -- audit a PKCS#11 module for post-quantum readiness, and write the
...
Complete the four functions marked TODO. `PQ_LAB_IMPL=solution pytest labs/ch10/tests` runs the reference.
"""
...
def parse_header(path: Path = HEADER) -> dict[str, dict]:
    """TODO: regex over #define CK[MKPFA]_NAME value lines of pkcs11t.h; group into CKM/CKK/CKP/CKF/CKA tables; first definition of a value wins"""
    raise NotImplementedError("parse_header")
...
def flag_names(flags: int, ckf: dict[str, int]) -> list[str]:
    """TODO: return the CKF_* names from the wanted list whose bit is set in flags"""
    raise NotImplementedError("flag_names")
...
def finish(rep: TokenReport) -> TokenReport:
    """TODO: compute required_missing and recommended_missing (absent, or missing flags) and set the verdict: READY / NOT PORTABLE (vendor-defined present) / NOT READY"""
...
def requirements_line(rep: TokenReport) -> str:
    """TODO: one sentence for the RFP naming PKCS#11 3.2, the REQUIRED mechanisms under 3.2 numbers, C_EncapsulateKey, CKA_PARAMETER_SET, CKA_SEED, FIPS 140-3 L3 with PQ inside the boundary, and the audited gap"""
    raise NotImplementedError("requirements_line")
