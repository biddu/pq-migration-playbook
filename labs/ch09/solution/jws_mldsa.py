"""Lab 9.2 solution -- ML-DSA-signed JWTs (RFC 9964) and the header budget they have to fit.
...
def verify(jwk: dict, token: str) -> dict:
    """Verify a compact JWS against a public JWK; return the payload claims."""
    h64, p64, s64 = token.split(".")
    header = json.loads(b64d(h64))
    sig, signing_input = b64d(s64), f"{h64}.{p64}".encode()
    alg = header["alg"]
    if alg != jwk.get("alg", alg) and jwk["kty"] == "AKP":
        raise ValueError("alg mismatch between header and AKP key")
    if alg in MLDSA:
        pub = getattr(mldsa, f"{alg.replace('-', '')}PublicKey").from_public_bytes(b64d(jwk["pub"]))
        pub.verify(sig, signing_input)
    elif alg == "ES256":
        from cryptography.hazmat.primitives.asymmetric.utils import encode_dss_signature
        pub = ec.EllipticCurvePublicNumbers(int.from_bytes(b64d(jwk["x"]), "big"), int.from_bytes(b64d(jwk["y"]), "big"), ec.SECP256R1()).public_key()
        pub.verify(encode_dss_signature(int.from_bytes(sig[:32], "big"), int.from_bytes(sig[32:], "big")), signing_input, ec.ECDSA(hashes.SHA256()))
    else:
        ed25519.Ed25519PublicKey.from_public_bytes(b64d(jwk["x"])).verify(sig, signing_input)
    return json.loads(b64d(p64))
...
def issue(signer: Signer, claims: dict, extra_header: dict | None = None) -> str:
    header = {"alg": signer.alg, "typ": "JWT", "kid": signer.kid, **(extra_header or {})}
    h64, p64 = b64u(json.dumps(header, separators=(",", ":")).encode()), b64u(json.dumps(claims, separators=(",", ":")).encode())
    return f"{h64}.{p64}.{b64u(signer.sign(f'{h64}.{p64}'.encode()))}"


def fits(token_bytes: int) -> dict:
    """Which budgets an 'Authorization: Bearer <token>' header (or a cookie) fits."""
    header_line = len("Authorization: Bearer ") + token_bytes + 2
    return {name: header_line <= limit for name, limit in BUDGETS.items()}
